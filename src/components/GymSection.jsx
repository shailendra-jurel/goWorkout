...
const GymSection = () => {
  ...
  return (
    <div className="gym-section max-w-4xl mx-auto p-8 bg-white shadow-xl rounded-lg">
      <h2 className="text-4xl font-extrabold mb-10 text-gray-800 text-center">Gym Workout Tracker</h2>
      {data.map((row, dateIndex) => (
        <div key={dateIndex} className="mb-10 bg-gray-50 p-6 rounded-lg shadow-md">
          <div className="flex items-center justify-between mb-4">
            ...
            >
              <FaTrash className="w-5 h-5" /> {/* Trash icon for removing date */}
            </button>
          </div>
          ...
          {row.exercises.map((exercise, exerciseIndex) => (
            <div key={exerciseIndex} className="flex mb-4 items-center">
              ...
              <button
                ...
                className="ml-4 text-red-600 hover:text-red-800 transition duration-300"
              >
                <FaTrash className="w-5 h-5" /> {/* Trash icon for removing exercise */}
              </button>
            </div>
          ))}
          <button
